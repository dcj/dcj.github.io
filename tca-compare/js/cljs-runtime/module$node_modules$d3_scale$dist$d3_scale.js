...
function(_){if(!arguments.length)return domain.slice();domain=[];index=new Map;for(var $jscomp$iter$319=$jscomp.makeIterator(_),$jscomp$key$value=$jscomp$iter$319.next();!$jscomp$key$value.done;$jscomp$key$value=$jscomp$iter$319.next()){$jscomp$key$value=$jscomp$key$value.value;var key=$jscomp$key$value+"";index.has(key)||index.set(key,domain.push($jscomp$key$value))}return scale};scale.range=function(_){return arguments.length?(range=Array.from(_),scale):range.slice()};scale.unknown=function(_){return arguments.length?
...
i/n);return scale}function scale(x){return isNaN(x=+x)?unknown:range[d3Array.bisect(thresholds,x)]}var domain=[],range=[],thresholds=[],unknown;scale.invertExtent=function(y){y=range.indexOf(y);return 0>y?[NaN,NaN]:[0<y?thresholds[y-1]:domain[0],y<thresholds.length?thresholds[y]:domain[domain.length-1]]};scale.domain=function(_){if(!arguments.length)return domain.slice();domain=[];for(var $jscomp$iter$320=$jscomp.makeIterator(_),$jscomp$key$d=$jscomp$iter$320.next();!$jscomp$key$d.done;$jscomp$key$d=
$jscomp$iter$320.next())$jscomp$key$d=$jscomp$key$d.value,null==$jscomp$key$d||isNaN($jscomp$key$d=+$jscomp$key$d)||domain.push($jscomp$key$d);domain.sort(d3Array.ascending);return rescale()};scale.range=function(_){return arguments.length?(range=Array.from(_),rescale()):range.slice()};scale.unknown=function(_){return arguments.length?(unknown=_,scale):unknown};scale.quantiles=function(){return thresholds.slice()};scale.copy=function(){return quantile().domain(domain).range(range).unknown(unknown)};
