...
i,value$jscomp$0[i]);else add(name$jscomp$0,-1,value$jscomp$0)}this.marshall().clear();initonly&&(argops.initonly=!0);return deps},marshall:function(stamp){var argval=this._argval||NO_PARAMS,argops=this._argops,i;if(argops){var n=argops.length;for(i=0;i<n;++i){var item=argops[i];var op=item.op;var mod=op.modified()&&op.stamp===stamp;argval.set(item.name,item.index,op.value,mod)}if(argops.initonly){for(i=0;i<n;++i)item=argops[i],item.op.targets().remove(this);this._update=this._argops=null}}return argval},
detach:function(){var argops=this._argops,n;if(argops){var i=0;for(n=argops.length;i<n;++i){var item=argops[i];item=item.op;item._targets&&item._targets.remove(this)}}},evaluate:function(pulse){var update=this._update;if(update){var params=this.marshall(pulse.stamp);update=update.call(this,params,pulse);params.clear();if(update!==this.value)this.value=update;else if(!this.modified())return pulse.StopPropagation}},run:function(pulse){if(pulse.stamp<this.stamp)return pulse.StopPropagation;if(this.skip()){this.skip(!1);
