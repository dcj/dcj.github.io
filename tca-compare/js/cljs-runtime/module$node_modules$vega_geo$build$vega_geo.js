...
_.smooth),tz=_.thresholds||levels(pulse,field,_),as=null===_.as?null:_.as||"contour",values=[];pulse.forEach(function(t){var grid=field(t),paths=contour.size([grid.width,grid.height])(grid.values,vegaUtil.isArray(tz)?tz:tz(grid.values));transformPaths(paths,grid,t,_);paths.forEach(function(p){var $jscomp$compprop50={};values.push(vegaDataflow.rederive(t,vegaDataflow.ingest(null!=as?($jscomp$compprop50[as]=p,$jscomp$compprop50):p)))})});this.value&&(out.rem=this.value);this.value=out.source=out.add=
