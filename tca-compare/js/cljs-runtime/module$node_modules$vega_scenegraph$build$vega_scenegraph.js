...
item,x,y){return rectShape.context(context)(item,x,y)}function clip$jscomp$0(renderer,item,size){var clip=item.clip;renderer=renderer._defs;item=item.clip_id||(item.clip_id="clip"+clip_id++);renderer=renderer.clipping[item]||(renderer.clipping[item]={id:item});vegaUtil.isFunction(clip)?renderer.path=clip(null):hasCornerRadius(size)?renderer.path=rectangle(null,size,0,0):(renderer.width=size.width||0,renderer.height=size.height||0);return"url(#"+item+")"}function Bounds(b){this.clear();b&&this.union(b)}
function Item(mark){this.mark=mark;this.bounds=this.bounds||new Bounds}function GroupItem(mark){Item.call(this,mark);this.items=this.items||[]}function ResourceLoader(customLoader){this._pending=0;this._loader=customLoader||vegaLoader.loader()}function boundStroke(bounds,item,miter){if(item.stroke&&0!==item.opacity&&0!==item.strokeOpacity){var sw=null!=item.strokeWidth?+item.strokeWidth:1;bounds.expand(sw+(miter?item.strokeJoin&&"miter"!==item.strokeJoin?0:sw:0))}return bounds}function boundContext(_,
deg){bounds=_;deg?(rot=deg*DegToRad,ma=md=Math.cos(rot),mb=Math.sin(rot),mc=-mb):(ma=md=1,rot=mb=mc=0);return context$jscomp$0}function quadExtrema(x0,x1,x2,cb){x2=(x0-x1)/(x0+x2-2*x1);0<x2&&1>x2&&cb(x0+(x1-x0)*x2)}function cubicExtrema(x0,x1,x2,x3,cb){var a=x3-x0+3*x1-3*x2,b=x0+x2-2*x1,c=x0-x1,t0=0,t1=0;1E-14<Math.abs(a)?(c=b*b+c*a,0<=c&&(c=Math.sqrt(c),t0=(-b+c)/a,t1=(-b-c)/a)):t0=.5*c/b;0<t0&&1>t0&&cb(cubic(t0,x0,x1,x2,x3));0<t1&&1>t1&&cb(cubic(t1,x0,x1,x2,x3))}function cubic(t,x0,x1,x2,x3){var s=
1-t,s2=s*s,t2=t*t;return s2*s*x0+3*s2*t*x1+3*s*t2*x2+t2*t*x3}function intersectPath(draw){return function(item,brush){if(!context$1)return!0;draw(context$1,item);b$jscomp$0.clear().union(item.bounds).intersect(brush).round();var $jscomp$destructuring$var432=b$jscomp$0;item=$jscomp$destructuring$var432.x1;brush=$jscomp$destructuring$var432.x2;var y2=$jscomp$destructuring$var432.y2;for($jscomp$destructuring$var432=$jscomp$destructuring$var432.y1;$jscomp$destructuring$var432<=y2;++$jscomp$destructuring$var432)for(var x$530=
item;x$530<=brush;++x$530)if(context$1.isPointInPath(x$530,$jscomp$destructuring$var432))return!0;return!1}}function intersectPoint(item,box){return box.contains(item.x||0,item.y||0)}function intersectRect(item,box){var x=item.x||0,y=item.y||0;return box.intersects(b$jscomp$0.set(x,y,x+(item.width||0),y+(item.height||0)))}function intersectRule(item,box){var x=item.x||0,y=item.y||0;return intersectBoxLine(box,x,y,null!=item.x2?item.x2:x,null!=item.y2?item.y2:y)}function intersectBoxLine(box,x,y,u,
v){var x1=box.x1,y1=box.y1,x2=box.x2;box=box.y2;u-=x;v-=y;var t0=0,t1=1,e;for(e=0;4>e;++e){if(0===e){var p=-u;var q=-(x1-x)}1===e&&(p=u,q=x2-x);2===e&&(p=-v,q=-(y1-y));3===e&&(p=v,q=box-y);if(1E-10>Math.abs(p)&&0>q)return!1;var r=q/p;if(0>p){if(r>t1)return!1;r>t0&&(t0=r)}else if(0<p){if(r<t0)return!1;r<t1&&(t1=r)}}return!0}function blend(context,item){context.globalCompositeOperation=item.blend||"source-over"}function value$1(value,dflt){return null==value?dflt:value}function addStops(gradient,stops){for(var n=
stops.length,i=0;i<n;++i)gradient.addColorStop(stops[i].offset,stops[i].color);return gradient}function gradient$jscomp$0(context,spec,bounds){var w=bounds.width(),h=bounds.height();if("radial"===spec.gradient)context=context.createRadialGradient(bounds.x1+value$1(spec.x1,.5)*w,bounds.y1+value$1(spec.y1,.5)*h,Math.max(w,h)*value$1(spec.r1,0),bounds.x1+value$1(spec.x2,.5)*w,bounds.y1+value$1(spec.y2,.5)*h,Math.max(w,h)*value$1(spec.r2,.5));else{var x1=value$1(spec.x1,0),y1=value$1(spec.y1,0),x2=value$1(spec.x2,
1),y2=value$1(spec.y2,0);if(x1===x2||y1===y2||w===h)context=context.createLinearGradient(bounds.x1+x1*w,bounds.y1+y1*h,bounds.x1+x2*w,bounds.y1+y2*h);else{bounds=vegaCanvas.canvas(Math.ceil(w),Math.ceil(h));var ictx=bounds.getContext("2d");ictx.scale(w,h);ictx.fillStyle=addStops(ictx.createLinearGradient(x1,y1,x2,y2),spec.stops);ictx.fillRect(0,0,w,h);return context.createPattern(bounds,"no-repeat")}}return addStops(context,spec.stops)}function fill$jscomp$0(context,item,opacity){opacity*=null==item.fillOpacity?
1:item.fillOpacity;return 0<opacity?(context.globalAlpha=opacity,item=(opacity=item.fill)&&opacity.gradient?gradient$jscomp$0(context,opacity,item.bounds):opacity,context.fillStyle=item,!0):!1}function stroke$jscomp$0(context,item,opacity){var lw=null!=(lw=item.strokeWidth)?lw:1;if(0>=lw)return!1;opacity*=null==item.strokeOpacity?1:item.strokeOpacity;return 0<opacity?(context.globalAlpha=opacity,opacity=(opacity=item.stroke)&&opacity.gradient?gradient$jscomp$0(context,opacity,item.bounds):opacity,
context.strokeStyle=opacity,context.lineWidth=lw,context.lineCap=item.strokeCap||"butt",context.lineJoin=item.strokeJoin||"miter",context.miterLimit=item.strokeMiterLimit||10,context.setLineDash&&(context.setLineDash(item.strokeDash||Empty),context.lineDashOffset=item.strokeDashOffset||0),!0):!1}function compare(a,b){return a.zindex-b.zindex||a.index-b.index}function zorder(scene){if(!scene.zdirty)return scene.zitems;var items=scene.items,output=[],n;var i=0;for(n=items.length;i<n;++i){var item=items[i];
item.index=i;item.zindex&&output.push(item)}scene.zdirty=!1;return scene.zitems=output.sort(compare)}function visit(scene,visitor){var items=scene.items,n;if(items&&items.length){var zitems=zorder(scene);if(zitems&&zitems.length){scene=0;for(n=items.length;scene<n;++scene)items[scene].zindex||visitor(items[scene]);items=zitems}scene=0;for(n=items.length;scene<n;++scene)visitor(items[scene])}}function pickVisit(scene,visitor){var items=scene.items,hit,i;if(!items||!items.length)return null;var zitems=
zorder(scene);zitems&&zitems.length&&(items=zitems);for(i=items.length;0<=--i;)if(hit=visitor(items[i]))return hit;if(items===zitems)for(items=scene.items,i=items.length;0<=--i;)if(!items[i].zindex&&(hit=visitor(items[i])))return hit;return null}function drawAll(path){return function(context,scene,bounds){visit(scene,function(item){bounds&&!bounds.intersects(item.bounds)||drawPath(path,context,item,item)})}}function drawOne(path){return function(context,scene,bounds){!scene.items.length||bounds&&
!bounds.intersects(scene.bounds)||drawPath(path,context,scene.items[0],scene.items)}}function drawPath(path,context,item,items){var opacity=null==item.opacity?1:item.opacity;0===opacity||path(context,items)||(blend(context,item),item.fill&&fill$jscomp$0(context,item,opacity)&&context.fill(),item.stroke&&stroke$jscomp$0(context,item,opacity)&&context.stroke())}function pick(test){test=test||vegaUtil.truthy;return function(context,scene,x,y,gx,gy){x*=context.pixelRatio;y*=context.pixelRatio;return pickVisit(scene,
function(item){var b=item.bounds;if((!b||b.contains(gx,gy))&&b&&test(context,item,x,y,gx,gy))return item})}}function hitPath(path,filled){return function(context,o,x,y){var item=Array.isArray(o)?o[0]:o,fill=null==filled?item.fill:filled,stroke=item.stroke&&context.isPointInStroke;if(stroke){var lw=item.strokeWidth;item=item.strokeCap;context.lineWidth=null!=lw?lw:1;context.lineCap=null!=item?item:"butt"}return path(context,o)?!1:fill&&context.isPointInPath(x,y)||stroke&&context.isPointInStroke(x,
y)}}function translate(x,y){return"translate("+x+","+y+")"}function markItemPath(type,shape,isect){function draw(context,item){var x=item.x||0,y=item.y||0,a=item.angle||0;context.translate(x,y);a&&context.rotate(a*=DegToRad);context.beginPath();shape(context,item);a&&context.rotate(-a);context.translate(-x,-y)}return{type:type,tag:"path",nested:!1,attr:function(emit,item){var JSCompiler_inline_result=translate(item.x||0,item.y||0)+(item.angle?" rotate("+(item.angle+")"):"");emit("transform",JSCompiler_inline_result);
emit("d",shape(null,item))},bound:function(bounds,item){shape(boundContext(bounds,item.angle),item);return boundStroke(bounds,item).translate(item.x||0,item.y||0)},draw:drawAll(draw),pick:pick(hitPath(draw)),isect:isect||intersectPath(draw)}}function markMultiItemPath(type,shape,tip){function draw(context,items){context.beginPath();shape(context,items)}var hit=hitPath(draw);return{type:type,tag:"path",nested:!0,attr:function(emit,item){item=item.mark.items;item.length&&emit("d",shape(null,item))},
bound:function(bounds,mark){mark=mark.items;if(0===mark.length)return bounds;shape(boundContext(bounds),mark);return boundStroke(bounds,mark[0])},draw:drawOne(draw),pick:function(context,scene,x,y,gx,gy){var items=scene.items;scene=scene.bounds;if(!items||!items.length||scene&&!scene.contains(gx,gy))return null;x*=context.pixelRatio;y*=context.pixelRatio;return hit(context,items,x,y)?items[0]:null},isect:intersectPoint,tip:tip}}function clipGroup(context,group){context.beginPath();hasCornerRadius(group)?
rectangle(context,group,0,0):context.rect(0,0,group.width||0,group.height||0);context.clip()}function offset$jscomp$0(item){var sw=value$1(item.strokeWidth,1);return null!=item.strokeOffset?item.strokeOffset:item.stroke&&.5<sw&&1.5>sw?.5-Math.abs(sw-1):0}function rectanglePath(context,group,x,y){var off=offset$jscomp$0(group);context.beginPath();rectangle(context,group,(x||0)+off,(y||0)+off)}function getImage(item,renderer){var image$jscomp$0=item.image;if(!image$jscomp$0||item.url&&item.url!==image$jscomp$0.url)image$jscomp$0=
{complete:!1,width:0,height:0},renderer.loadImage(item.url).then(function(image){item.image=image;item.image.url=item.url});return image$jscomp$0}function imageWidth(item,image){return null!=item.width?item.width:image&&image.width?!1!==item.aspect&&item.height?item.height*image.width/image.height:image.width:0}function imageHeight(item,image){return null!=item.height?item.height:image&&image.height?!1!==item.aspect&&item.width?item.width*image.height/image.width:image.height:0}function imageXOffset(align,
w){return"center"===align?w/2:"right"===align?w:0}function imageYOffset(baseline,h){return"middle"===baseline?h/2:"bottom"===baseline?h:0}function path$jscomp$0(context,item){var path=item.path;if(null==path)return!0;var x=item.x||0,y=item.y||0,sx=item.scaleX||1,sy=item.scaleY||1,a=(item.angle||0)*DegToRad,cache=item.pathCache;cache&&cache.path===path||((item.pathCache=cache=pathParse(path)).path=path);a&&context.rotate&&context.translate?(context.translate(x,y),context.rotate(a),pathRender(context,
cache,0,0,sx,sy),context.rotate(-a),context.translate(-x,-y)):pathRender(context,cache,x,y,sx,sy)}function draw$2(context,item){context.beginPath();rectangle(context,item)}function path$2(context,item,opacity){if(item.stroke&&stroke$jscomp$0(context,item,opacity)){opacity=item.x||0;var y1=item.y||0;var x2=null!=item.x2?item.x2:opacity;item=null!=item.y2?item.y2:y1;context.beginPath();context.moveTo(opacity,y1);context.lineTo(x2,item);return!0}return!1}function useCanvas(use){textMetrics.width=use&&
context$1?measureWidth:estimateWidth}function estimateWidth(item,text){text=textValue(item,text);item=fontSize(item);return~~(.8*text.length*item)}function measureWidth(item,text){return 0>=fontSize(item)||!(text=textValue(item,text))?0:_measureWidth(text,font(item))}function _measureWidth(text,currentFont){var key="("+currentFont+") "+text,width=widthCache.get(key);void 0===width&&(context$1.font=currentFont,width=context$1.measureText(text).width,widthCache.set(key,width));return width}function fontSize(item){return null!=
item.fontSize?+item.fontSize||0:11}function lineHeight(item){return null!=item.lineHeight?item.lineHeight:fontSize(item)+2}function textLines(item){item=item.lineBreak&&item.text&&!vegaUtil.isArray(item.text)?item.text.split(item.lineBreak):item.text;return vegaUtil.isArray(item)?1<item.length?item:item[0]:item}function textValue(item,line){line=null==line?"":(line+"").trim();if(0<item.limit&&line.length){var limit=+item.limit,width=widthGetter(item);if(!(width(line)<limit)){var ellipsis=item.ellipsis||
"…",rtl="rtl"===item.dir;item=0;var hi=line.length;limit-=width(ellipsis);if(rtl){for(;item<hi;)rtl=item+hi>>>1,width(line.slice(rtl))>limit?item=rtl+1:hi=rtl;line=ellipsis+line.slice(item)}else{for(;item<hi;)rtl=1+(item+hi>>>1),width(line.slice(0,rtl))<limit?item=rtl:hi=rtl-1;line=line.slice(0,item)+ellipsis}}}return line}function widthGetter(item){if(textMetrics.width===measureWidth){var currentFont=font(item);return function(text){return _measureWidth(text,currentFont)}}var currentFontHeight=fontSize(item);
return function(text){return~~(.8*text.length*currentFontHeight)}}function fontFamily(item,quote){item=item.font;return(quote&&item?String(item).replace(/"/g,"'"):item)||"sans-serif"}function font(item,quote){return(item.fontStyle?item.fontStyle+" ":"")+(item.fontVariant?item.fontVariant+" ":"")+(item.fontWeight?item.fontWeight+" ":"")+fontSize(item)+"px "+fontFamily(item,quote)}function offset$1(item){var baseline=item.baseline,h=fontSize(item);return Math.round("top"===baseline?.79*h:"middle"===
baseline?.3*h:"bottom"===baseline?-.21*h:"line-top"===baseline?.29*h+.5*lineHeight(item):"line-bottom"===baseline?.29*h-.5*lineHeight(item):0)}function anchorPoint(item){var x=item.x||0,y=item.y||0,r=item.radius||0;r&&(item=(item.theta||0)-HalfPi,x+=r*Math.cos(item),y+=r*Math.sin(item));tempBounds.x1=x;tempBounds.y1=y;return tempBounds}function bound$5(bounds,item,mode){var h=textMetrics.height(item),a=item.align,p=anchorPoint(item),x=p.x1;p=p.y1;var dx=item.dx||0,dy=(item.dy||0)+offset$1(item)-Math.round(.8*
h),tl=textLines(item);vegaUtil.isArray(tl)?(h+=lineHeight(item)*(tl.length-1),tl=tl.reduce(function(w,t){return Math.max(w,textMetrics.width(item,t))},0)):tl=textMetrics.width(item,tl);"center"===a?dx-=tl/2:"right"===a&&(dx-=tl);bounds.set(dx+=x,dy+=p,dx+tl,dy+h);if(item.angle&&!mode)bounds.rotate(item.angle*DegToRad,x,p);else if(2===mode)return bounds.rotatedPoints(item.angle*DegToRad,x,p);return bounds}function boundItem(item,func,opt){var type=Marks[item.mark.marktype];func=func||type.bound;type.nested&&
(item=item.mark);return func(item.bounds||(item.bounds=new Bounds),item,opt)}function boundMark(mark,bounds,opt){var type=Marks[mark.marktype],bound=type.bound,items=mark.items,hasItems=items&&items.length;if(type.nested)return hasItems?mark=items[0]:(DUMMY.mark=mark,mark=DUMMY),opt=boundItem(mark,bound,opt),bounds=bounds&&bounds.union(opt)||opt;bounds=bounds||mark.bounds&&mark.bounds.clear()||new Bounds;if(hasItems)for(type=0,hasItems=items.length;type<hasItems;++type)bounds.union(boundItem(items[type],
...
return scene}function Scenegraph(scene){arguments.length?this.root=sceneFromJSON(scene):(this.root=createMark({marktype:"group",name:"root",role:"frame"}),this.root.items=[new GroupItem(this.root)])}function createMark(def,group){group={bounds:new Bounds,clip:!!def.clip,group:group,interactive:!1===def.interactive?!1:!0,items:[],marktype:def.marktype,name:def.name||void 0,role:def.role||void 0,zindex:def.zindex||0};null!=def.aria&&(group.aria=def.aria);def.description&&(group.description=def.description);
...
type||"touchend"===type?["touchstart","touchmove","touchend"]:[type]).forEach(function(_){return addEventListener(handler,_)})}function addEventListener(handler,type){var canvas=handler.canvas();canvas&&!handler._events[type]&&(handler._events[type]=1,canvas.addEventListener(type,handler[type]?function(evt){return handler[type](evt)}:function(evt){return handler.fire(type,evt)}))}function move(moveEvent,overEvent,outEvent){return function(evt){var a=this._active,p=this.pickEvent(evt);p!==a&&(a&&a.exit||
this.fire(outEvent,evt),this._active=p,this.fire(overEvent,evt));this.fire(moveEvent,evt)}}function inactive(type){return function(evt){this.fire(type,evt);this._active=null}}function CanvasRenderer(loader){Renderer.call(this,loader);this._options={};this._redraw=!1;this._dirty=new Bounds;this._tempb=new Bounds}function SVGHandler(loader,tooltip){Handler.call(this,loader,tooltip);var h=this;h._hrefHandler=listener(h,function(evt,item){item&&item.href&&h.handleHref(evt,item,item.href)});h._tooltipHandler=
listener(h,function(evt,item){h.handleTooltip(evt,item,"mouseout"!==evt.type)})}function ariaItemAttributes(emit,item){var hide=!1===item.aria;emit("aria-hidden",hide||void 0);if(hide||null==item.description)for(var prop in AriaEncode)emit(AriaEncode[prop],void 0);else hide=item.mark.marktype,emit("aria-label",item.description),emit("role",item.ariaRole||("group"===hide?"graphics-object":"graphics-symbol")),emit("aria-roledescription",item.ariaRoleDescription||hide+" mark")}function ariaMarkAttributes(mark){var $jscomp$compprop49=
{};return!1===mark.aria?($jscomp$compprop49["aria-hidden"]=!0,$jscomp$compprop49):AriaIgnore[mark.role]?null:AriaGuides[mark.role]?ariaGuide(mark,AriaGuides[mark.role]):ariaMark(mark)}function ariaMark(mark){var type=mark.marktype,recurse="group"===type||"text"===type||mark.items.some(function(_){return null!=_.description&&!1!==_.aria});return bundle(recurse?"graphics-object":"graphics-symbol",type+" mark container",mark.description)}function ariaGuide(mark,opt){try{var item=mark.items[0],caption=
opt.caption||function(){return""};return bundle(opt.role||"graphics-symbol",opt.desc,item.description||caption(item))}catch(err){return null}}function extractTitle(item){try{return vegaUtil.array(vegaUtil.peek(item.items).items[0].text).join(" ")}catch(err){return null}}function channelCaption(props){props=props.map(function(p){return p+("fill"===p||"stroke"===p?" color":"")});return 2>props.length?props[0]:props.slice(0,-1).join(", ")+" and "+vegaUtil.peek(props)}function markup(){var buf="",outer=
"",inner="",stack=[],attr=function(name,value){null!=value&&(outer+=" "+name+'\x3d"'+innerText(value).replace(/"/g,"\x26quot;").replace(/\t/g,"\x26#x9;").replace(/\n/g,"\x26#xA;").replace(/\r/g,"\x26#xD;")+'"');return m},m={open:function(tag,attrs){for(var $jscomp$restParams=[],$jscomp$restIndex=1;$jscomp$restIndex<arguments.length;++$jscomp$restIndex)$jscomp$restParams[$jscomp$restIndex-1]=arguments[$jscomp$restIndex];outer&&(buf+=outer+"\x3e"+inner,outer=inner="");stack.push(tag);outer="\x3c"+tag;
$jscomp$restParams=$jscomp.makeIterator($jscomp$restParams);for($jscomp$restIndex=$jscomp$restParams.next();!$jscomp$restIndex.done;$jscomp$restIndex=$jscomp$restParams.next()){$jscomp$restIndex=$jscomp$restIndex.value;for(var key in $jscomp$restIndex)attr(key,$jscomp$restIndex[key])}return m},close:function(){var tag=stack.pop();buf=outer?buf+(outer+(inner?"\x3e"+inner+"\x3c/"+tag+"\x3e":"/\x3e")):buf+("\x3c/"+tag+"\x3e");outer=inner="";return m},attr:attr,text:function(t){return inner+=innerText(t),
m},toString:function(){return buf}};return m}function _serialize(m,node){m.open(node.tagName);if(node.hasAttributes())for(var attrs=node.attributes,n=attrs.length,i=0;i<n;++i)m.attr(attrs[i].name,attrs[i].value);if(node.hasChildNodes())for(node=node.childNodes,attrs=node.length,n=0;n<attrs;n++)i=node[n],3===i.nodeType?m.text(i.nodeValue):_serialize(m,i);return m.close()}function SVGRenderer(loader){Renderer.call(this,loader);this._dirtyID=0;this._dirty=[];this._defs=this._root=this._svg=null}function dirtyParents(item,
id){for(;item&&item.dirty!==id;item=item.mark.group)if(item.dirty=id,item.mark&&item.mark.dirty!==id)item.mark.dirty=id;else break}function recurse$jscomp$0(renderer,el,group){el=el.lastChild.previousSibling;var prev,idx=0;visit(group,function(item){prev=renderer.mark(el,item,prev);++idx});domClear(el,1+idx)}function bind(item,el,sibling,tag,svg){var node=item._svg;if(!node){var doc=el.ownerDocument;node=domCreate(doc,tag,ns);item._svg=node;item.mark&&(node.__data__=item,node.__values__={fill:"default"},
"g"===tag&&(tag=domCreate(doc,"path",ns),node.appendChild(tag),tag.__data__=item,tag=domCreate(doc,"g",ns),node.appendChild(tag),tag.__data__=item,doc=domCreate(doc,"path",ns),node.appendChild(doc),doc.__data__=item,doc.__values__={fill:"default"}))}(node.ownerSVGElement!==svg||node.parentNode&&1<node.parentNode.childNodes.length&&node.previousSibling!=sibling)&&el.insertBefore(node,sibling?sibling.nextSibling:el.firstChild);return node}function emit(name,value,ns){if(value!==values[name]){if(ns){var el=
element;null!=value?el.setAttributeNS(ns,name,value):el.removeAttributeNS(ns,name)}else setAttribute(element,name,value);values[name]=value}}function setStyle(el,name,value){value!==values[name]&&(null==value?el.style.removeProperty(name):el.style.setProperty(name,value+""),values[name]=value)}function setAttributes(el,attrs){for(var key in attrs)setAttribute(el,key,attrs[key])}function setAttribute(el,name,value){null!=value?el.setAttribute(name,value):el.removeAttribute(name)}function href(){var loc;
return"undefined"===typeof window?"":(loc=window.location).hash?loc.href.slice(0,-loc.hash.length):loc.href}function SVGStringRenderer(loader){Renderer.call(this,loader);this._text=null;this._defs={gradient:{},clipping:{}}}function style(s,item,scene,tag,defs){if(null==item)return s;"bgrect"===tag&&!1===scene.interactive&&(s["pointer-events"]="none");if("bgfore"===tag&&(!1===scene.interactive&&(s["pointer-events"]="none"),s.display="none",null!==item.fill))return s;"image"===tag&&!1===item.smooth&&
(s.style="image-rendering: optimizeSpeed; image-rendering: pixelated;");"text"===tag&&(s["font-family"]=fontFamily(item),s["font-size"]=fontSize(item)+"px",s["font-style"]=item.fontStyle,s["font-variant"]=item.fontVariant,s["font-weight"]=item.fontWeight);for(var prop in styles)scene=item[prop],tag=styles[prop],("transparent"!==scene||"fill"!==tag&&"stroke"!==tag)&&null!=scene&&(scene&&scene.gradient&&(scene=gradientRef(scene,defs.gradient,"")),s[tag]=scene);return s}function intersectMark(mark,box,
filter,hits){if(mark.bounds&&box.intersects(mark.bounds)&&("group"===mark.marktype||!1!==mark.interactive&&(!filter||filter(mark)))){var items=mark.items,type$548=mark.marktype;mark=items.length;var i=0;if("group"===type$548)for(;i<mark;++i)intersectGroup(items[i],box,filter,hits);else for(filter=Marks[type$548].isect;i<mark;++i)type$548=items[i],intersectItem(type$548,box,filter)&&hits.push(type$548)}return hits}function intersectGroup(group,box,filter,hits){filter&&filter(group.mark)&&intersectItem(group,
box,Marks.group.isect)&&hits.push(group);var marks=group.items,n=marks&&marks.length;if(n){var x$549=group.x||0;group=group.y||0;box.translate(-x$549,-group);for(var i=0;i<n;++i)intersectMark(marks[i],box,filter,hits);box.translate(x$549,group)}return hits}function intersectItem(item,box,test){var bounds=item.bounds;return box.encloses(bounds)||box.intersects(bounds)&&test(item,box)}function sceneEqual(a,b,key){return a===b?!0:"path"===key?pathEqual(a,b):a instanceof Date&&b instanceof Date?+a===
+b:vegaUtil.isNumber(a)&&vegaUtil.isNumber(b)?1E-9>=Math.abs(a-b):a&&b&&(vegaUtil.isObject(a)||vegaUtil.isObject(b))?objectEqual(a,b):a==b}function pathEqual(a,b){return sceneEqual(pathParse(a),pathParse(b))}function objectEqual(a,b){var ka=Object.keys(a),kb=Object.keys(b),i;if(ka.length!==kb.length)return!1;ka.sort();kb.sort();for(i=ka.length-1;0<=i;i--)if(ka[i]!=kb[i])return!1;for(i=ka.length-1;0<=i;i--)if(kb=ka[i],!sceneEqual(a[kb],b[kb],kb))return!1;return typeof a===typeof b}var gradient_id=
...
/###/,/(\.\d+)(\.\d)/g,/(\d)([-+])/g,/\s|,|###/],DegToRad=Math.PI/180,HalfPi=Math.PI/2,Tau=2*Math.PI,HalfSqrt3=Math.sqrt(3)/2,segmentCache={},bezierCache={},join=[].join,temp=["l",0,0,0,0,0,0,0],builtins={circle:{draw:function(context,size){size=Math.sqrt(size)/2;context.moveTo(size,0);context.arc(0,0,size,0,Tau)}},cross:{draw:function(context,size){size=Math.sqrt(size)/2;var s=size/2.5;context.moveTo(-size,-s);context.lineTo(-size,s);context.lineTo(-s,s);context.lineTo(-s,size);context.lineTo(s,
...
"triangle-right":{draw:function(context,size){size=Math.sqrt(size)/2;var h=HalfSqrt3*size;context.moveTo(h,0);context.lineTo(-h,-size);context.lineTo(-h,size);context.closePath()}},"triangle-left":{draw:function(context,size){size=Math.sqrt(size)/2;var h=HalfSqrt3*size;context.moveTo(-h,0);context.lineTo(h,-size);context.lineTo(h,size);context.closePath()}},stroke:{draw:function(context,size){size=Math.sqrt(size)/2;context.moveTo(-size,0);context.lineTo(size,0)}}},custom={},x$jscomp$0=function(item){return item.x||
0},y$jscomp$0=function(item){return item.y||0},def$jscomp$0=function(item){return!1!==item.defined},arcShape=d3Shape.arc().startAngle(function(item){return item.startAngle||0}).endAngle(function(item){return item.endAngle||0}).padAngle(function(item){return item.padAngle||0}).innerRadius(function(item){return item.innerRadius||0}).outerRadius(function(item){return item.outerRadius||0}).cornerRadius(function(item){return item.cornerRadius||0}),areavShape=d3Shape.area().x(x$jscomp$0).y1(y$jscomp$0).y0(function(item){return(item.y||
0)+(item.height||0)}).defined(def$jscomp$0),areahShape=d3Shape.area().y(y$jscomp$0).x1(x$jscomp$0).x0(function(item){return(item.x||0)+(item.width||0)}).defined(def$jscomp$0),lineShape=d3Shape.line().x(x$jscomp$0).y(y$jscomp$0).defined(def$jscomp$0),rectShape=vg_rect().x(x$jscomp$0).y(y$jscomp$0).width(function(item){return item.width||0}).height(function(item){return item.height||0}).cornerRadius(function(item){return value$jscomp$0(item.cornerRadiusTopLeft,item.cornerRadius)||0},function(item){return value$jscomp$0(item.cornerRadiusTopRight,
item.cornerRadius)||0},function(item){return value$jscomp$0(item.cornerRadiusBottomRight,item.cornerRadius)||0},function(item){return value$jscomp$0(item.cornerRadiusBottomLeft,item.cornerRadius)||0}),symbolShape=d3Shape.symbol().type(function(item){return symbols(item.shape||"circle")}).size(function(item){return value$jscomp$0(item.size,64)}),trailShape=vg_trail().x(x$jscomp$0).y(y$jscomp$0).defined(def$jscomp$0).size(function(item){return item.size||1}),clip_id=1;Bounds.prototype={clone:function(){return new Bounds(this)},
clear:function(){this.x1=+Number.MAX_VALUE;this.y1=+Number.MAX_VALUE;this.x2=-Number.MAX_VALUE;this.y2=-Number.MAX_VALUE;return this},empty:function(){return this.x1===+Number.MAX_VALUE&&this.y1===+Number.MAX_VALUE&&this.x2===-Number.MAX_VALUE&&this.y2===-Number.MAX_VALUE},equals:function(b){return this.x1===b.x1&&this.y1===b.y1&&this.x2===b.x2&&this.y2===b.y2},set:function(x1,y1,x2,y2){x2<x1?(this.x2=x1,this.x1=x2):(this.x1=x1,this.x2=x2);y2<y1?(this.y2=y1,this.y1=y2):(this.y1=y1,this.y2=y2);return this},
add:function(x,y){x<this.x1&&(this.x1=x);y<this.y1&&(this.y1=y);x>this.x2&&(this.x2=x);y>this.y2&&(this.y2=y);return this},expand:function(d){this.x1-=d;this.y1-=d;this.x2+=d;this.y2+=d;return this},round:function(){this.x1=Math.floor(this.x1);this.y1=Math.floor(this.y1);this.x2=Math.ceil(this.x2);this.y2=Math.ceil(this.y2);return this},scale:function(s){this.x1*=s;this.y1*=s;this.x2*=s;this.y2*=s;return this},translate:function(dx,dy){this.x1+=dx;this.x2+=dx;this.y1+=dy;this.y2+=dy;return this},
rotate:function(angle,x,y){angle=this.rotatedPoints(angle,x,y);return this.clear().add(angle[0],angle[1]).add(angle[2],angle[3]).add(angle[4],angle[5]).add(angle[6],angle[7])},rotatedPoints:function(angle,x,y){var x1=this.x1,y1=this.y1,x2=this.x2,y2=this.y2,cos=Math.cos(angle);angle=Math.sin(angle);var cx=x-x*cos+y*angle;x=y-x*angle-y*cos;return[cos*x1-angle*y1+cx,angle*x1+cos*y1+x,cos*x1-angle*y2+cx,angle*x1+cos*y2+x,cos*x2-angle*y1+cx,angle*x2+cos*y1+x,cos*x2-angle*y2+cx,angle*x2+cos*y2+x]},union:function(b){b.x1<
this.x1&&(this.x1=b.x1);b.y1<this.y1&&(this.y1=b.y1);b.x2>this.x2&&(this.x2=b.x2);b.y2>this.y2&&(this.y2=b.y2);return this},intersect:function(b){b.x1>this.x1&&(this.x1=b.x1);b.y1>this.y1&&(this.y1=b.y1);b.x2<this.x2&&(this.x2=b.x2);b.y2<this.y2&&(this.y2=b.y2);return this},encloses:function(b){return b&&this.x1<=b.x1&&this.x2>=b.x2&&this.y1<=b.y1&&this.y2>=b.y2},alignsWith:function(b){return b&&(this.x1==b.x1||this.x2==b.x2||this.y1==b.y1||this.y2==b.y2)},intersects:function(b){return b&&!(this.x2<
b.x1||this.x1>b.x2||this.y2<b.y1||this.y1>b.y2)},contains:function(x,y){return!(x<this.x1||x>this.x2||y<this.y1||y>this.y2)},width:function(){return this.x2-this.x1},height:function(){return this.y2-this.y1}};vegaUtil.inherits(GroupItem,Item);ResourceLoader.prototype={pending:function(){return this._pending},sanitizeURL:function(uri){var loader=this;loader._pending+=1;return loader._loader.sanitize(uri,{context:"href"}).then(function(opt){--loader._pending;return opt}).catch(function(){--loader._pending;
...
width:0,height:0,src:e&&e.url||""}})},ready:function(){var loader=this;return new Promise(function(accept){function poll(value){loader.pending()?setTimeout(function(){poll(!0)},10):accept(value)}poll(!1)})}};var circleThreshold=Tau-1E-8,bounds,lx,ly,rot,ma,mb,mc,md,addL=function(x,y){x=lx=x;y=ly=y;return bounds.add(x,y)},addX=function(x){return bounds.add(x,bounds.y1)},addY=function(y){return bounds.add(bounds.x1,y)},addp=function(x,y){return bounds.add(ma*x+mc*y,mb*x+md*y)},addpL=function(x,y){return addL(ma*
x+mc*y,mb*x+md*y)},context$jscomp$0={beginPath:function(){},closePath:function(){},moveTo:addpL,lineTo:addpL,rect:function(x,y,w,h){rot?(addp(x+w,y),addp(x+w,y+h),addp(x,y+h),addpL(x,y)):(bounds.add(x+w,y+h),addL(x,y))},quadraticCurveTo:function(x1,y1,x2,y2){var py1=mb*x1+md*y1,px2=ma*x2+mc*y2;x2=mb*x2+md*y2;quadExtrema(lx,ma*x1+mc*y1,px2,addX);quadExtrema(ly,py1,x2,addY);addL(px2,x2)},bezierCurveTo:function(x1,y1,x2,y2,x3,y3){var py1=mb*x1+md*y1,py2=mb*x2+md*y2,px3=ma*x3+mc*y3;x3=mb*x3+md*y3;cubicExtrema(lx,
ma*x1+mc*y1,ma*x2+mc*y2,px3,addX);cubicExtrema(ly,py1,py2,x3,addY);addL(px3,x3)},arc:function(cx,cy,r,sa,ea,ccw){sa+=rot;ea+=rot;lx=r*Math.cos(ea)+cx;ly=r*Math.sin(ea)+cy;if(Math.abs(ea-sa)>circleThreshold)bounds.add(cx-r,cy-r),bounds.add(cx+r,cy+r);else{var update=function(a){return bounds.add(r*Math.cos(a)+cx,r*Math.sin(a)+cy)};update(sa);update(ea);if(ea!==sa){sa%=Tau;0>sa&&(sa+=Tau);ea%=Tau;0>ea&&(ea+=Tau);if(ea<sa){ccw=!ccw;var s=sa;sa=ea;ea=s}if(ccw)for(ea-=Tau,s=sa-sa%HalfPi,sa=0;4>sa&&s>ea;++sa,
s-=HalfPi)update(s);else for(s=sa-sa%HalfPi+HalfPi,sa=0;4>sa&&s<ea;++sa,s+=HalfPi)update(s)}}}},context$1=(context$1=vegaCanvas.canvas(1,1))?context$1.getContext("2d"):null,b$jscomp$0=new Bounds,Empty=[],arc$1=markItemPath("arc",function(context,item){return arcShape.context(context)(item)}),area$1=markMultiItemPath("area",function(context,items){var item=items[0];return("horizontal"===item.orient?areahShape:areavShape).curve(curves(item.interpolate||"linear",item.orient,item.tension)).context(context)(items)},
function(a,p){p="horizontal"===a[0].orient?p[1]:p[0];for(var z="horizontal"===a[0].orient?"y":"x",i=a.length,min=Infinity,hit,d;0<=--i;)!1!==a[i].defined&&(d=Math.abs(a[i][z]-p),d<min&&(min=d,hit=a[i]));return hit}),hitBackground=hitPath(rectanglePath),hitForeground=hitPath(rectanglePath,!1),hitCorner=hitPath(rectanglePath,!0),group$jscomp$0={type:"group",tag:"g",nested:!1,attr:function(emit,item){emit("transform",translate(item.x||0,item.y||0))},bound:function(bounds,group){if(!group.clip&&group.items)for(var items=
group.items,m=items.length,j=0;j<m;++j)bounds.union(items[j].bounds);(group.clip||group.width||group.height)&&!group.noBound&&bounds.add(0,0).add(group.width||0,group.height||0);boundStroke(bounds,group);return bounds.translate(group.x||0,group.y||0)},draw:function(context,scene,bounds){var $jscomp$this=this;visit(scene,function(group){var gx=group.x||0,gy=group.y||0,fore=group.strokeForeground,opacity=null==group.opacity?1:group.opacity;(group.stroke||group.fill)&&opacity&&(rectanglePath(context,
group,gx,gy),blend(context,group),group.fill&&fill$jscomp$0(context,group,opacity)&&context.fill(),group.stroke&&!fore&&stroke$jscomp$0(context,group,opacity)&&context.stroke());context.save();context.translate(gx,gy);group.clip&&clipGroup(context,group);bounds&&bounds.translate(-gx,-gy);visit(group,function(item){$jscomp$this.draw(context,item,bounds)});bounds&&bounds.translate(gx,gy);context.restore();fore&&group.stroke&&opacity&&(rectanglePath(context,group,gx,gy),blend(context,group),stroke$jscomp$0(context,
group,opacity)&&context.stroke())})},pick:function(context,scene,x,y,gx,gy){var $jscomp$this=this;if(scene.bounds&&!scene.bounds.contains(gx,gy)||!scene.items)return null;var cx=x*context.pixelRatio,cy=y*context.pixelRatio;return pickVisit(scene,function(group){var hit=group.bounds;if(!hit||hit.contains(gx,gy)){var dx=group.x||0;var dy=group.y||0;hit=dx+(group.width||0);var dh=dy+(group.height||0),c=group.clip;if(!c||!(gx<dx||gx>hit||gy<dy||gy>dh)){context.save();context.translate(dx,dy);dx=gx-dx;
dy=gy-dy;if(c&&hasCornerRadius(group)&&!hitCorner(context,group,cx,cy))return context.restore(),null;dh=group.strokeForeground;if((c=!1!==scene.interactive)&&dh&&group.stroke&&hitForeground(context,group,cx,cy))return context.restore(),group;hit=pickVisit(group,function(mark){return(!1!==mark.interactive||"group"===mark.marktype)&&mark.bounds&&mark.bounds.contains(dx,dy)?$jscomp$this.pick(mark,x,y,dx,dy):null});!hit&&c&&(group.fill||!dh&&group.stroke)&&hitBackground(context,group,cx,cy)&&(hit=group);
context.restore();return hit||null}}})},isect:intersectRect,content:function(emit,item,renderer){item=item.clip?clip$jscomp$0(renderer,item,item):null;emit("clip-path",item)},background:function(emit,item){emit("class","background");emit("aria-hidden",!0);var off=offset$jscomp$0(item);emit("d",rectangle(null,item,off,off))},foreground:function(emit,item){emit("class","foreground");emit("aria-hidden",!0);if(item.strokeForeground){var off=offset$jscomp$0(item);emit("d",rectangle(null,item,off,off))}else emit("d",
"")}},metadata={xmlns:"http://www.w3.org/2000/svg","xmlns:xlink":"http://www.w3.org/1999/xlink",version:"1.1"},image$jscomp$1={type:"image",tag:"image",nested:!1,attr:function(emit,item,renderer){var img=getImage(item,renderer);renderer=imageWidth(item,img);var h=imageHeight(item,img),x=(item.x||0)-imageXOffset(item.align,renderer),y=(item.y||0)-imageYOffset(item.baseline,h);img=!img.src&&img.toDataURL?img.toDataURL():img.src||"";emit("href",img,metadata["xmlns:xlink"],"xlink:href");emit("transform",
translate(x,y));emit("width",renderer);emit("height",h);emit("preserveAspectRatio",!1===item.aspect?"none":"xMidYMid")},bound:function(bounds,item){var img=item.image,w=imageWidth(item,img);img=imageHeight(item,img);var x=(item.x||0)-imageXOffset(item.align,w);item=(item.y||0)-imageYOffset(item.baseline,img);return bounds.set(x,item,x+w,item+img)},draw:function(context,scene,bounds){var $jscomp$this=this;visit(scene,function(item){if(!bounds||bounds.intersects(item.bounds)){var img=getImage(item,
$jscomp$this),w=imageWidth(item,img),h=imageHeight(item,img),x=(item.x||0)-imageXOffset(item.align,w),y=(item.y||0)-imageYOffset(item.baseline,h),opacity;if(!1!==item.aspect){var ar0=img.width/img.height;var ar1=item.width/item.height;ar0===ar0&&ar1===ar1&&ar0!==ar1&&(ar1<ar0?(ar0=w/ar0,y+=(h-ar0)/2,h=ar0):(ar0*=h,x+=(w-ar0)/2,w=ar0))}if(img.complete||img.toDataURL)blend(context,item),context.globalAlpha=null!=(opacity=item.opacity)?opacity:1,context.imageSmoothingEnabled=!1!==item.smooth,context.drawImage(img,
x,y,w,h)}})},pick:pick(),isect:vegaUtil.truthy,get:getImage,xOffset:imageXOffset,yOffset:imageYOffset},line$1=markMultiItemPath("line",function(context,items){var item=items[0];return lineShape.curve(curves(item.interpolate||"linear",item.orient,item.tension)).context(context)(items)},function(a,p){for(var t=Math.pow(a[0].strokeWidth||1,2),i=a.length,dx,dy;0<=--i;)if(!1!==a[i].defined&&(dx=a[i].x-p[0],dy=a[i].y-p[1],dx=dx*dx+dy*dy,dx<t))return a[i];return null}),path$1={type:"path",tag:"path",nested:!1,
attr:function(emit,item){var sy=item.scaleY||1;1===(item.scaleX||1)&&1===sy||emit("vector-effect","non-scaling-stroke");sy=translate(item.x||0,item.y||0)+(item.angle?" rotate("+(item.angle+")"):"")+(item.scaleX||item.scaleY?" scale("+((item.scaleX||1)+","+(item.scaleY||1)+")"):"");emit("transform",sy);emit("d",item.path)},bound:function(bounds,item){return path$jscomp$0(boundContext(bounds,item.angle),item)?bounds.set(0,0,0,0):boundStroke(bounds,item,!0)},draw:drawAll(path$jscomp$0),pick:pick(hitPath(path$jscomp$0)),
isect:intersectPath(path$jscomp$0)},rect$jscomp$0={type:"rect",tag:"path",nested:!1,attr:function(emit,item){emit("d",rectangle(null,item))},bound:function(bounds,item){var x,y;return boundStroke(bounds.set(x=item.x||0,y=item.y||0,x+item.width||0,y+item.height||0),item)},draw:drawAll(draw$2),pick:pick(hitPath(draw$2)),isect:intersectRect},rule={type:"rule",tag:"line",nested:!1,attr:function(emit,item){emit("transform",translate(item.x||0,item.y||0));emit("x2",null!=item.x2?item.x2-(item.x||0):0);
emit("y2",null!=item.y2?item.y2-(item.y||0):0)},bound:function(bounds,item){var x1,y1;return boundStroke(bounds.set(x1=item.x||0,y1=item.y||0,null!=item.x2?item.x2:x1,null!=item.y2?item.y2:y1),item)},draw:function(context,scene,bounds){visit(scene,function(item){if(!bounds||bounds.intersects(item.bounds)){var opacity=null==item.opacity?1:item.opacity;opacity&&path$2(context,item,opacity)&&(blend(context,item),context.stroke())}})},pick:pick(function(context,item,x,y){return context.isPointInStroke?
path$2(context,item,1)&&context.isPointInStroke(x,y):!1}),isect:intersectRule},shape$1=markItemPath("shape",function(context,item){return(item.mark.shape||item.shape).context(context)(item)}),symbol$1=markItemPath("symbol",function(context,item){return symbolShape.context(context)(item)},intersectPoint),widthCache=vegaUtil.lruCache(),textMetrics={height:fontSize,measureWidth:measureWidth,estimateWidth:estimateWidth,width:estimateWidth,canvas:useCanvas};useCanvas(!0);var textAlign={left:"start",center:"middle",
right:"end"},tempBounds=new Bounds,text$jscomp$0={type:"text",tag:"text",nested:!1,attr:function(emit,item){var dx=item.dx||0,dy=(item.dy||0)+offset$1(item),p=anchorPoint(item),x=p.x1;p=p.y1;var a=item.angle||0;emit("text-anchor",textAlign[item.align]||"start");if(a){if(item=translate(x,p)+" rotate("+(a+")"),dx||dy)item+=" "+translate(dx,dy)}else item=translate(x+dx,p+dy);emit("transform",item)},bound:bound$5,draw:function(context,scene,bounds){visit(scene,function(item){var opacity=null==item.opacity?
1:item.opacity,i;if(!(bounds&&!bounds.intersects(item.bounds)||0===opacity||0>=item.fontSize||null==item.text||0===item.text.length)){context.font=font(item);context.textAlign=item.align||"left";var p=anchorPoint(item);var x=p.x1;p=p.y1;item.angle&&(context.save(),context.translate(x,p),context.rotate(item.angle*DegToRad),x=p=0);x+=item.dx||0;p+=(item.dy||0)+offset$1(item);var tl=textLines(item);blend(context,item);if(vegaUtil.isArray(tl)){var lh=lineHeight(item);for(i=0;i<tl.length;++i){var str=
...
handler:handler,listener:listener(this,handler)},(h[name]||(h[name]=[])).push(type),this._svg&&this._svg.addEventListener(name,type.listener));return this},off:function(type,handler){var name=this.eventName(type),h=this._handlers[name];type=this._handlerIndex(h,type,handler);0<=type&&(this._svg&&this._svg.removeEventListener(name,h[type].listener),h.splice(type,1));return this}});var bundle=function(role,roledesc,label){var $jscomp$compprop48={};return $jscomp$compprop48.role=role,$jscomp$compprop48["aria-roledescription"]=
roledesc,$jscomp$compprop48["aria-label"]=label||void 0,$jscomp$compprop48},AriaIgnore=vegaUtil.toSet("axis-domain axis-grid axis-label axis-tick axis-title legend-band legend-entry legend-gradient legend-label legend-title legend-symbol title".split(" ")),AriaGuides={axis:{desc:"axis",caption:function(item){var datum=item.datum,orient=item.orient,title=datum.title?extractTitle(item):null,ctx=item.context;datum=ctx.scales[datum.scale].value;ctx=ctx.dataflow.locale();var type=datum.type;return("left"===
...
setAttribute(parent,"clip-path",scene.clip?clip$jscomp$0(this,scene,scene.group):null);events=function(item){var dirty=$jscomp$this.isDirty(item),node=bind(item,parent,sibling,mdef.tag,svg);dirty&&($jscomp$this._update(mdef,node,item),isGroup&&recurse$jscomp$0($jscomp$this,node,item));sibling=node;++i};mdef.nested?scene.items.length&&events(scene.items[0]):visit(scene,events);domClear(parent,i);return parent},_update:function(mdef,el,item){element=el;values=el.__values__;ariaItemAttributes(emit,item);
mdef.attr(emit,item,this);var extra=mark_extras[mdef.type];extra&&extra.call(this,mdef,el,item);element&&this.style(element,item)},style:function(el,item){if(null!=item)for(var prop in styles){var value$538="font"===prop?fontFamily(item):item[prop];if(value$538!==values[prop]){var name=styles[prop];null==value$538?el.removeAttribute(name):(value$538&&value$538.gradient&&(value$538=gradientRef(value$538,this._defs.gradient,href())),el.setAttribute(name,value$538+""));values[prop]=value$538}}},defs:function(){var svg=
...
bg&&"none"!==bg&&m.open("rect",{width:this._width,height:this._height,fill:bg}).close();m.open("g",rootAttributes,{transform:"translate("+this._origin+")"});this.mark(m,scene);m.close();this.defs(m);this._text=m.close()+"";return this},mark:function(m,scene$jscomp$0){var $jscomp$this=this,mdef=Marks[scene$jscomp$0.marktype],tag=mdef.tag,attrList=[ariaItemAttributes,mdef.attr];m.open("g",{"class":cssClass(scene$jscomp$0),"clip-path":scene$jscomp$0.clip?clip$jscomp$0(this,scene$jscomp$0,scene$jscomp$0.group):
null},ariaMarkAttributes(scene$jscomp$0),{"pointer-events":"g"!==tag&&!1===scene$jscomp$0.interactive?"none":null});var process=function(item){var href=$jscomp$this.href(item);href&&m.open("a",href);m.open(tag,$jscomp$this.attr(scene$jscomp$0,item,attrList,"g"!==tag?tag:null));if("text"===tag){var tl$541=textLines(item);if(vegaUtil.isArray(tl$541))for(var attrs={x:0,dy:lineHeight(item)},i=0;i<tl$541.length;++i)m.open("tspan",i?attrs:null).text(textValue(item,tl$541[i])).close();else m.text(textValue(item,
tl$541))}else"g"===tag&&(tl$541=item.strokeForeground,attrs=item.fill,i=item.stroke,tl$541&&i&&(item.stroke=null),m.open("path",$jscomp$this.attr(scene$jscomp$0,item,mdef.background,"bgrect")).close(),m.open("g",$jscomp$this.attr(scene$jscomp$0,item,mdef.content)),visit(item,function(scene){return $jscomp$this.mark(m,scene)}),m.close(),tl$541&&i?(attrs&&(item.fill=null),item.stroke=i,m.open("path",$jscomp$this.attr(scene$jscomp$0,item,mdef.foreground,"bgrect")).close(),attrs&&(item.fill=attrs)):m.open("path",
...
textMetrics;Object.defineProperty(exports,"__esModule",{value:!0})})}
