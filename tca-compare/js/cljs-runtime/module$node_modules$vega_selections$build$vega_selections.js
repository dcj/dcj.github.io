...
"union";Object.keys(resolved).forEach(function(field){resolved[field]=Object.keys(resolved[field]).map(function(unit){return resolved[field][unit]}).reduce(function(acc,curr){return void 0===acc?curr:ops[types[field]+"_"+op](acc,curr)})});name=Object.keys(multiRes);isMulti&&name.length&&(isMulti={},unit$jscomp$0={},resolved.vlMulti="union"===op?(isMulti.or=name.reduce(function(acc,k){return acc.push.apply(acc,multiRes[k]),acc},[]),isMulti):(unit$jscomp$0.and=name.map(function(k){var $jscomp$compprop54=
{};return $jscomp$compprop54.or=multiRes[k],$jscomp$compprop54}),unit$jscomp$0));return resolved};exports.selectionTest=function(name,datum,op){var data=this.context.data[name];name=data?data.values.value:[];data=data?data["index:unit"]&&data["index:unit"].value:void 0;op="intersect"===op;for(var n=name.length,i=0,entry,miss,count,unit;i<n;++i)if(entry=name[i],data&&op){if(miss=miss||{},count=miss[unit=entry.unit]||0,-1!==count){entry=testPoint(datum,entry);miss[unit]=entry?-1:++count;if(entry&&1===
...
"unit"));vegaUtil.hasOwnProperty(params,dataName)||(params[dataName]=scope.getData(name).tuplesRef())};Object.defineProperty(exports,"__esModule",{value:!0})})}
