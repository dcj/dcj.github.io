...
y1,dy){var y$jscomp$0=d3Array.range(y0,y1-1E-6,dy).concat(y1);return function(x){return y$jscomp$0.map(function(y){return[x,y]})}}function graticuleY(x0,x1,dx){var x$jscomp$0=d3Array.range(x0,x1-1E-6,dx).concat(x1);return function(y){return x$jscomp$0.map(function(x){return[x,y]})}}function graticule$jscomp$0(){function graticule(){return{type:"MultiLineString",coordinates:lines()}}function lines(){return d3Array.range(ceil(X0/DX)*DX,X1,DX).map(X).concat(d3Array.range(ceil(Y0/DY)*DY,Y1,DY).map(Y)).concat(d3Array.range(ceil(x0/
...
mercatorRaw?[[Math.max(t[0]-k,x0),y0],[Math.min(t[0]+k,x1),y1]]:[[x0,Math.max(t[1]-k,y0)],[x1,Math.min(t[1]+k,y1)]])}var m=projection$jscomp$0(project),center=m.center,scale=m.scale,translate=m.translate,clipExtent=m.clipExtent,x0=null,y0,x1,y1;m.scale=function(_){return arguments.length?(scale(_),reclip()):scale()};m.translate=function(_){return arguments.length?(translate(_),reclip()):translate()};m.center=function(_){return arguments.length?(center(_),reclip()):center()};m.clipExtent=function(_){return arguments.length?
...
width,object)};albersUsa.fitHeight=function(height,object){return fitHeight(albersUsa,height,object)};return albersUsa.scale(1070)};exports.geoArea=function(object){areaSum=new d3Array.Adder;geoStream(object,areaStream);return 2*areaSum};exports.geoAzimuthalEqualArea=function(){return projection$jscomp$0(azimuthalEqualAreaRaw).scale(124.75).clipAngle(179.999)};exports.geoAzimuthalEqualAreaRaw=azimuthalEqualAreaRaw;exports.geoAzimuthalEquidistant=function(){return projection$jscomp$0(azimuthalEquidistantRaw).scale(79.4188).clipAngle(179.999)};
...
containsObjectType[object.type]:containsGeometry)(object,point)};exports.geoDistance=distance;exports.geoEqualEarth=function(){return projection$jscomp$0(equalEarthRaw).scale(177.158)};exports.geoEqualEarthRaw=equalEarthRaw;exports.geoEquirectangular=function(){return projection$jscomp$0(equirectangularRaw).scale(152.63)};exports.geoEquirectangularRaw=equirectangularRaw;exports.geoGnomonic=function(){return projection$jscomp$0(gnomonicRaw).scale(144.049).clipAngle(60)};exports.geoGnomonicRaw=gnomonicRaw;
exports.geoGraticule=graticule$jscomp$0;exports.geoGraticule10=function(){return graticule$jscomp$0()()};exports.geoIdentity=function(){function reset(){kx=k*sx;ky=k*sy;cache=cacheStream=null;return projection}function projection(p){var x=p[0]*kx;p=p[1]*ky;if(alpha){var t=p*ca-x*sa;x=x*ca+p*sa;p=t}return[x+tx,p+ty]}var k=1,tx=0,ty=0,sx=1,sy=1,alpha=0,ca,sa,x0=null,y0,x1,y1,kx=1,ky=1,transform=transformer({point:function(x,y){x=projection([x,y]);this.stream.point(x[0],x[1])}}),postclip=identity,cache,
cacheStream;projection.invert=function(p){var x=p[0]-tx;p=p[1]-ty;if(alpha){var t=p*ca+x*sa;x=x*ca-p*sa;p=t}return[x/kx,p/ky]};projection.stream=function(stream){return cache&&cacheStream===stream?cache:cache=transform(postclip(cacheStream=stream))};projection.postclip=function(_){return arguments.length?(postclip=_,x0=y0=x1=y1=null,reset()):postclip};projection.clipExtent=function(_){return arguments.length?(postclip=null==_?(x0=y0=x1=y1=null,identity):clipRectangle(x0=+_[0][0],y0=+_[0][1],x1=+_[1][0],
y1=+_[1][1]),reset()):null==x0?null:[[x0,y0],[x1,y1]]};projection.scale=function(_){return arguments.length?(k=+_,reset()):k};projection.translate=function(_){return arguments.length?(tx=+_[0],ty=+_[1],reset()):[tx,ty]};projection.angle=function(_){return arguments.length?(alpha=_%360*radians,sa=sin(alpha),ca=cos(alpha),reset()):alpha*degrees};projection.reflectX=function(_){return arguments.length?(sx=_?-1:1,reset()):0>sx};projection.reflectY=function(_){return arguments.length?(sy=_?-1:1,reset()):
0>sy};projection.fitExtent=function(extent,object){return fitExtent(projection,extent,object)};projection.fitSize=function(size,object){return fitExtent(projection,[[0,0],size],object)};projection.fitWidth=function(width,object){return fitWidth(projection,width,object)};projection.fitHeight=function(height,object){return fitHeight(projection,height,object)};return projection};exports.geoInterpolate=function(a,b){var x0=a[0]*radians,y0=a[1]*radians;a=b[0]*radians;b=b[1]*radians;var cy0=cos(y0),sy0=
sin(y0),cy1=cos(b),sy1=sin(b),kx0=cy0*cos(x0),ky0=cy0*sin(x0),kx1=cy1*cos(a),ky1=cy1*sin(a),d=2*asin(sqrt(haversin(b-y0)+cy0*cy1*haversin(a-x0))),k=sin(d);a=d?function(t){var B=sin(t*=d)/k,A=sin(d-t)/k;t=A*kx0+B*kx1;var y=A*ky0+B*ky1;B=A*sy0+B*sy1;return[atan2(y,t)*degrees,atan2(B,sqrt(t*t+y*y))*degrees]}:function(){return[x0*degrees,y0*degrees]};a.distance=d;return a};exports.geoLength=length$jscomp$0;exports.geoMercator=function(){return mercatorProjection(mercatorRaw).scale(961/tau)};exports.geoMercatorRaw=
mercatorRaw;exports.geoNaturalEarth1=function(){return projection$jscomp$0(naturalEarth1Raw).scale(175.295)};exports.geoNaturalEarth1Raw=naturalEarth1Raw;exports.geoOrthographic=function(){return projection$jscomp$0(orthographicRaw).scale(249.5).clipAngle(90.000001)};exports.geoOrthographicRaw=orthographicRaw;exports.geoPath=function(projection,context){function path(object){object&&("function"===typeof pointRadius&&contextStream.pointRadius(+pointRadius.apply(this,arguments)),geoStream(object,projectionStream(contextStream)));
...
+_);return path};return path.projection(projection).context(context)};exports.geoProjection=projection$jscomp$0;exports.geoProjectionMutator=projectionMutator;exports.geoRotation=rotation$jscomp$0;exports.geoStereographic=function(){return projection$jscomp$0(stereographicRaw).scale(250).clipAngle(142)};exports.geoStereographicRaw=stereographicRaw;exports.geoStream=geoStream;exports.geoTransform=function(methods){return{stream:transformer(methods)}};exports.geoTransverseMercator=function(){var m=
