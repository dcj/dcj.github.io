...
sumValue*sumValue*alpha;newRatio=Math.max(maxValue/newRatio,newRatio/minValue);if(newRatio>minRatio){sumValue-=nodeValue;break}minRatio=newRatio}rows.push(row={value:sumValue,dice:parent<dy,children:nodes.slice(row,i1)});row.dice?treemapDice(row,x0,y0,x1,value?y0+=dy*sumValue/value:y1):treemapSlice(row,x0,y0,value?x0+=parent*sumValue/value:x1,y1);value-=sumValue;row=i1}return rows}$jscomp.initSymbol();$jscomp.initSymbolIterator();var $jscomp$compprop52={};Node.prototype=hierarchy.prototype=($jscomp$compprop52.constructor=
Node,$jscomp$compprop52.count=function(){return this.eachAfter(count)},$jscomp$compprop52.each=function(callback,that){for(var index=-1,$jscomp$iter$323=$jscomp.makeIterator(this),$jscomp$key$node=$jscomp$iter$323.next();!$jscomp$key$node.done;$jscomp$key$node=$jscomp$iter$323.next())callback.call(that,$jscomp$key$node.value,++index,this);return this},$jscomp$compprop52.eachAfter=function(callback,that){for(var node,nodes=[this],next=[],i,n,index=-1;node=nodes.pop();)if(next.push(node),node=node.children)for(i=
0,n=node.length;i<n;++i)nodes.push(node[i]);for(;node=next.pop();)callback.call(that,node,++index,this);return this},$jscomp$compprop52.eachBefore=function(callback,that){for(var node,nodes=[this],i,index=-1;node=nodes.pop();)if(callback.call(that,node,++index,this),node=node.children)for(i=node.length-1;0<=i;--i)nodes.push(node[i]);return this},$jscomp$compprop52.find=function(callback,that){for(var index=-1,$jscomp$iter$324=$jscomp.makeIterator(this),$jscomp$key$node=$jscomp$iter$324.next();!$jscomp$key$node.done;$jscomp$key$node=
$jscomp$iter$324.next())if($jscomp$key$node=$jscomp$key$node.value,callback.call(that,$jscomp$key$node,++index,this))return $jscomp$key$node},$jscomp$compprop52.sum=function(value){return this.eachAfter(function(node){for(var sum=+value(node.data)||0,children=node.children,i=children&&children.length;0<=--i;)sum+=children[i].value;node.value=sum})},$jscomp$compprop52.sort=function(compare){return this.eachBefore(function(node){node.children&&node.children.sort(compare)})},$jscomp$compprop52.path=
function(end){var start=this;var ancestor=start;var b=end;if(ancestor!==b){var aNodes=ancestor.ancestors(),bNodes=b.ancestors(),c=null;ancestor=aNodes.pop();for(b=bNodes.pop();ancestor===b;)c=ancestor,ancestor=aNodes.pop(),b=bNodes.pop();ancestor=c}for(b=[start];start!==ancestor;)start=start.parent,b.push(start);for(start=b.length;end!==ancestor;)b.splice(start,0,end),end=end.parent;return b},$jscomp$compprop52.ancestors=function(){for(var node=this,nodes=[node];node=node.parent;)nodes.push(node);
return nodes},$jscomp$compprop52.descendants=function(){return Array.from(this)},$jscomp$compprop52.leaves=function(){var leaves=[];this.eachBefore(function(node){node.children||leaves.push(node)});return leaves},$jscomp$compprop52.links=function(){var root=this,links=[];root.each(function(node){node!==root&&links.push({source:node.parent,target:node})});return links},$jscomp$compprop52.copy=function(){return hierarchy(this).eachBefore(copyData)},$jscomp$compprop52[Symbol.iterator]=node_iterator,
$jscomp$compprop52);var preroot={depth:-1},ambiguous={};TreeNode.prototype=Object.create(Node.prototype);$jscomp$compprop52=(1+Math.sqrt(5))/2;var squarify=function custom(ratio){function squarify(parent,x0,y0,x1,y1){squarifyRatio(ratio,parent,x0,y0,x1,y1)}squarify.ratio=function(x){return custom(1<(x=+x)?x:1)};return squarify}($jscomp$compprop52);$jscomp$compprop52=function custom(ratio){function resquarify(parent,x0,y0,x1,y1){if((rows=parent._squarify)&&rows.ratio===ratio)for(var rows,nodes,i,j=
