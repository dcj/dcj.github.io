...
count:counts};if(!countOnly)for(var n$jscomp$0=measures.length,a=cell.agg=Array(n$jscomp$0),i$jscomp$0=0;i$jscomp$0<n$jscomp$0;++i$jscomp$0)a[i$jscomp$0]=new measures[i$jscomp$0](cell);if(cell.store)var store=cell.data=new TupleStore;cell.add=function(t){cell.num+=1;if(!countOnly){store&&store.add(t);for(var i$522=0;i$522<n$jscomp$0;++i$522)a[i$522].add(a[i$522].get(t),t)}};cell.rem=function(t){--cell.num;if(!countOnly){store&&store.rem(t);for(var i$523=0;i$523<n$jscomp$0;++i$523)a[i$523].rem(a[i$523].get(t),
t)}};cell.set=function(t){var n;store&&store.values();var i=0;for(n=counts.length;i<n;++i)t[counts[i]]=cell.num;if(!countOnly)for(i=0,n=a.length;i<n;++i)a[i].set(t)};cell.init=function(){cell.num=0;store&&store.reset();for(var i$524=0;i$524<n$jscomp$0;++i$524)a[i$524].init()};return cell}function Window(params){vegaDataflow.Transform.call(this,{},params);this._mlen=0;this._mods=[]}function processPartition(list,state,cmp,_){var sort=_.sort,range=sort&&!_.ignorePeers;_=_.frame||[null,0];list=list.data(cmp);
...
max=-Infinity,i=0,j;i<n;++i){var g=groups[i].sort(sort);j=-1;for(var $jscomp$iter$317=$jscomp.makeIterator(vegaStatistics.dotbin(g,source,smooth,field)),$jscomp$key$v=$jscomp$iter$317.next();!$jscomp$key$v.done;$jscomp$key$v=$jscomp$iter$317.next())$jscomp$key$v=$jscomp$key$v.value,$jscomp$key$v<min&&(min=$jscomp$key$v),$jscomp$key$v>max&&(max=$jscomp$key$v),g[++j][_]=$jscomp$key$v}this.value={start:min,stop:max,step:source};return pulse.reflow(!0).modifies(_)}});vegaUtil.inherits(Expression,vegaDataflow.Operator);
...
_.field);var names=(_.groupby||[]).map(vegaUtil.accessorName),values=[],step=_.step||.01,p=_.probs||d3Array.range(step/2,1-1E-14,step),n=p.length;pulse.forEach(function(g){for(var q=vegaStatistics.quantiles(g,p),i=0;i<n;++i){for(var t={},i$521=0;i$521<names.length;++i$521)t[names[i$521]]=g.dims[i$521];t[as[0]]=p[i];t[as[1]]=q[i];values.push(vegaDataflow.ingest(t))}});this.value&&(out.rem=this.value);this.value=out.add=out.source=values;return out}});vegaUtil.inherits(Relay,vegaDataflow.Transform,
